...
export default config => ({
  ...
  deployPlugins: [
    [
      ...
      {
        prepareCmd: `yarn prepublishOnly && yarn prepublishOnly firefox && zip -r dist/chrome.zip dist/chrome && zip -r zip -r dist/firefox.zip dist/firefox && git archive --output=dist/firefox-sources.zip HEAD && yarn ${packageName`publish-browser-extension`} --chrome-zip=dist/chrome.zip --firefox-zip=dist/firefox.zip --firefox-sources=dist/firefox-sources.zip`,
      },
    ],
  ],
